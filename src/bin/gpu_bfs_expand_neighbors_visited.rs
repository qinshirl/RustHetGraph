...
fn main() -> cust::error::CudaResult<()> {
    // cargo run --release --bin gpu_bfs_expand_neighbors_visited -- <bin_dir> [root_old]
    let out_dir = std::env::args().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        eprintln!("Usage: cargo run --release --bin gpu_bfs_expand_neighbors_visited -- <bin_dir> [root_old]");
        std::process::exit(2);
    });
    ...
    // CPU visited reference (only root marked visited for now)
    ...
    let f_write_filt = module.get_function("frontier_write_neighbors_filter_sentinel_u32")?;
    ...
    unsafe {
        launch!(
            f_write_filt<<<blocks, threads, 0, stream>>>(
                d_offsets.as_device_ptr(),
                d_dst.as_device_ptr(),
                d_frontier.as_device_ptr(),
    ...
    println!(
        "GPU pass2 (visited+sentinel) done: wrote {} slots, time={:.3} ms",
        total_out, pass2_ms
    );

    let mut expected_host: Vec<u32> = vec![0; total_out as usize];
    ...
    for i in 0..(frontier_len as usize) {
        ...
        for (j, &v) in g.dst[cpu_start..cpu_end].iter().enumerate() {
            expected_host[out_base + j] = if visited_cpu[v as usize] == 0 { v } else { SENTINEL };
        }
    }
    for (idx, (&gv, &cv)) in candidates_host.iter().zip(expected_host.iter()).enumerate() {
        if gv != cv {
            panic!("Mismatch at idx={idx}: gpu={gv}, cpu={cv}");
        }
    }

    let kept = candidates_host.iter().filter(|&&x| x != SENTINEL).count();
    let dropped = candidates_host.len() - kept;

    println!(
        "Validation OK. kept={}, dropped(sentinel)={}, total_slots={}",
        kept, dropped, candidates_host.len()
    );
